...
const oxygenScheama = new mongoose.Schema({
    ...
    b_total_oxygen_used_yesterday: {
        type: Number,
    },
    b_total_oxygen_received_yesterday: {
        type: Number,
    },
    b_total_oxygen_available: {
        type: Number,
    },
    d_total_oxygen_used_yesterday: {
        type: Number,
    },
    d_total_oxygen_received_yesterday: {
        type: Number,
    },
    d_total_oxygen_available: {
        type: Number,
    }
...
//                 save_pass:"swosth@ganjam$22"
//             }), "swosth@ganjam$22", function(err, user){
//                 if(err){
//                     console.log(err);
//                 }
//             });
...
//////////////////COVID_CELL///////////
app.get("/covid-cell-home/refresh", auth,HasRole("COVID_CELL"),c1,c2,c3,c4,c5, function (req, res) {res.redirect("/covid-cell-home")});
app.get("/covid-cell-home", auth, HasRole("COVID_CELL"), function (req, res) {
    Patient.find({}, async (err, patients_list) => {
        ...
        for (let index = 0; index < patients_list.length; index++) {
            ...
            if (element.discharge_status.toUpperCase().trim() === "LWMA") {
                lwma++;
            }
        ...
        Center.find({}, async (err, centers_list) => {
            ...
            for (let index = 0; index < centers_list.length; index++) {
                ...
                for (let j = 0; j < element.active_patient_ids.length; j++) {
                    ...
                    if (bed_d.bed_category === "HDU_BED") {
                        HDU_BED_COUNT++;
                    }
                    if (bed_d.bed_category === "ISOLATION_BED") {
                        ISOLATION_BED_COUNT++;
                    }
                ...
                hospital_list.push({
                    hospital: element,
                    total_bed_available: Number(element.no_general_bed + element.no_oxygen_bed + element.no_icu_bed + element.no_ventilator+ element.no_isolation_bed + element.no_hdu_bed),
                    ...
                    total_hdu_bed_available: element.no_hdu_bed,
                    total_hdu_bed_occupied: HDU_BED_COUNT,

                    total_isolation_bed_available: element.no_isolation_bed,
                    total_isolation_bed_occupied: ISOLATION_BED_COUNT,
            ...
            var total_hospital_data = {
                ...
                total_hdu_bed_available: 0,
                total_hdu_bed_occupied: 0,

                total_isolation_bed_available: 0,
                total_isolation_bed_occupied: 0,
            };
            var total_dch_data = {
                total_bed_available: 0,
                total_bed_occupied: 0,
            ...
            for (let index = 0; index < hospital_list.length; index++) {
                ...
                total_hospital_data.total_hdu_bed_available += element__.total_hdu_bed_available;
                total_hospital_data.total_hdu_bed_occupied += element__.total_hdu_bed_occupied;

                total_hospital_data.total_isolation_bed_available += element__.total_isolation_bed_available;
                total_hospital_data.total_isolation_bed_occupied += element__.total_isolation_bed_occupied;
                ...
                if (element__.hospital_category === "DCHC") {
                    ...
                    total_dchc_data.total_hdu_bed_available += element__.total_hdu_bed_available;
                    total_dchc_data.total_hdu_bed_occupied += element__.total_hdu_bed_occupied;
    
                    total_dchc_data.total_isolation_bed_available += element__.total_isolation_bed_available;
                    total_dchc_data.total_isolation_bed_occupied += element__.total_isolation_bed_occupied;
                }
                if (element__.hospital_category === "DCH") {
                    ...
                    total_dch_data.total_hdu_bed_available += element__.total_hdu_bed_available;
                    total_dch_data.total_hdu_bed_occupied += element__.total_hdu_bed_occupied;
    
                    total_dch_data.total_isolation_bed_available += element__.total_isolation_bed_available;
                    total_dch_data.total_isolation_bed_occupied += element__.total_isolation_bed_occupied;

                }
                
                if (element__.hospital_category === "CASUALTY") {
                    total_casualty_data.total_bed_available += element__.total_bed_available;
                    total_casualty_data.total_bed_occupied += element__.total_bed_occupied;
                    total_casualty_data.total_oxygen_bed_available += element__.total_oxygen_bed_available;
                    total_casualty_data.total_oxygen_bed_occupied += element__.total_oxygen_bed_occupied;
                    total_casualty_data.total_general_bed_available += element__.total_general_bed_available;
                    total_casualty_data.total_general_bed_occupied += element__.total_general_bed_occupied;
                    total_casualty_data.total_icu_bed_available += element__.total_icu_bed_available;
                    total_casualty_data.total_icu_bed_occupied += element__.total_icu_bed_occupied;
                    total_casualty_data.total_ventilator_bed_available += element__.total_ventilator_bed_available;
                    total_casualty_data.total_ventilator_bed_occupied += element__.total_ventilator_bed_occupied;

                    total_casualty_data.total_hdu_bed_available += element__.total_hdu_bed_available;
                    total_casualty_data.total_hdu_bed_occupied += element__.total_hdu_bed_occupied;
    
                    total_casualty_data.total_isolation_bed_available += element__.total_isolation_bed_available;
                    total_casualty_data.total_isolation_bed_occupied += element__.total_isolation_bed_occupied;
            ...
            res.render("COVID_CELL/hospital_dashboard", {
                ...
                total_casualty_data: total_casualty_data,
...
app.post('/edit-user', auth, HasRole("COVID_CELL"), (req, res) => {
    console.log(req.body);
...
app.post('/master_file_active_case_2', auth, HasRole("TECEADS"), (req, res) => {
...
//addpatient
app.post('/master_file_active_case', auth, HasRole("TECEADS"), (req, res) => {
    let sampleFile = req.files.file;
    var o_f_name = req.files.file.name
    console.log(req.files);
    var f_name = o_f_name.split(".");
        ...
        sampleFile.mv(path, function (err) {
            if (err) {
                ...
                console.log(result_);
                        ...
                        result_["Sheet1"].forEach(element => {
                            if (element.A >= 1) {
                                        ...
                                        Center.findOneAndUpdate({
                                            ...
                                            $push: {
                                                active_patient_ids: {
                                                    ...
                                                    bed_category: element.D
                                                }
                                            }
                                        }, (err, center_found) => {
                                            if(center_found){console.log(center_found.center_id);
                                            Sections.findOneAndUpdate({
                                                section_id: element.F
                                            }, {
                                                $push: {
                                                    active_patient_ids: {
                                                        patient_id: counterOut.prefix + counterOut.count,
                                                ...
                                                const patient = new Patient({
                                                    ...
                                                    // covid_report: req.body.covid_report,
                                                    co_morbidities:  element.P,
                                                    // travel_history: req.body.travel_history,
                                                    // positive_declare_date: req.body.positive_declare_date,
                                                    admission_date: element.C,
                                                    // referrals: req.body.referrals,
                                                    // symptoms: req.body.symptoms,
                            
                                                    previous_covid:  element.O,
                                                    // previous_covid_date: req.body.previous_covid_date,
                                                    // vaccination: req.body.vaccination,
                                                    // vaccination_date: req.body.vaccination_date,
                                                    // swap_collection_date: req.body.swap_collection_date,
                                                    // date_symptoms: req.body.date_symptoms,
                            
                                                    // "active_health_status.date": req.body.admission_date,
                                                    // "active_health_status.time": "Admission",
                                                    "active_health_status.section_id": element.F,
                                                    // "active_health_status.spo2": req.body.spo2,
                                                    // "active_health_status.fbs": req.body.fbs,
                                                    // "active_health_status.rbs": req.body.rbs,
                                                    ...
                                                    "health_status.section_id": [element.F],
                                                    // "health_status.spo2": [req.body.spo2],
                                                    // "health_status.fbs": [req.body.fbs],
                                                    // "health_status.rbs": [req.body.rbs],
                                                    ...
                                                    "active_bed_details.date": element.C,
                                                    "active_bed_details.center_id": element.B,
                                                    "active_bed_details.section_id": element.F,
                                                    ...
                                                    "active_bed_details.bed_category": element.D,
                                                    "active_bed_details.bed_no": element.E,
...
app.get("/hospitallist", auth,  HasRole("COVID_CELL"), function (req, res) {
    ...
    Center.find({}, async (err, centers_list) => {
        ...
        hospital_list = [{
            ...
            total_ventilator_bed_available: Number,
            total_ventilator_bed_occupied: Number,
            total_hdu_bed_occupied: Number,
            total_isolation_bed_occupied: Number,
        ...
        for (let index = 0; index < centers_list.length; index++) {
            ...
            hospital_list.push({
                hospital: element,
                total_bed_available: Number(element.no_general_bed + element.no_oxygen_bed + element.no_icu_bed + element.no_ventilator +  element.no_hdu_bed + element.no_isolation_bed),
                total_bed_occupied: element.active_patient_ids.length,
                total_oxygen_bed_occupied: OXYGEN_BED_COUNT,
                total_general_bed_occupied: GENERAL_BED_COUNT,
                total_icu_bed_occupied: ICU_BED_COUNT,
    ...
    Center.findOne({
        ...
        Sections.find({
            ...
            hospital_list = [{
                ...
                total_ventilator_bed_available: Number,
                total_ventilator_bed_occupied: Number,
                total_hdu_bed_available: Number,
                total_hdu_bed_occupied: Number,
                total_isolation_bed_available: Number,
                total_isolation_bed_occupied: Number,
            }];
            for (let index = 0; index < sections_list.length; index++) {
                ...
                hospital_list.push({
                    ...
                    section: element,
                    total_bed_available: Number(element.no_of_general_beds + element.no_of_oxygen_beds + element.no_of_icu_beds + element.no_of_ventilator_beds+element.no_isolation_beds+element.no_hdu_beds),
                    total_bed_occupied: element.active_patient_ids.length,
                    total_oxygen_bed_occupied: OXYGEN_BED_COUNT,
                    total_general_bed_occupied: GENERAL_BED_COUNT,
                    total_icu_bed_occupied: ICU_BED_COUNT,
            ...
            hospital_list.shift();
            console.log(hospital_list);
            res.render(req.user.department+"/section_list", {
                element:req.user,
...
app.get("/activepatientlist", auth,  HasRole("COVID_CELL"), function (req, res) {
    ...
    Patient.find({
        $or:[{discharge_status: "ACTIVE"},{discharge_status: "SHIFTING"}]
    }, (err, patients_list) => {
...
app.post('/accept_referral_rrt_covid_cell',  auth,  HasRole("COVID_CELL"), ( req, res) => {
    console.log(req.body)
    Center.findOne({
        center_id: req.body.to_center_id
    },(err, to_center_details) => {
    Referral.findOneAndUpdate({
        ...
        $set: {
            ...
            from_center_id: "COVID_CELL",
            to_center_details:to_center_details.facility_name,
        }
    }, (err, patient_updated) => {
...
app.post('/reject_referral_rrt_covid_cell',  auth,  HasRole("COVID_CELL"), ( req, res) => {
    Referral.findOneAndUpdate({
        referral_id: req.body.referral_id
    }, {
        $set: {
    ...
    Center.findOne({
        ...
        console.log(req.body)
            const oxygen = new Oxygen({
                center_id: centers_list.center_id,
                center_details: centers_list.center_details,
                date: req.body.date,
                b_total_oxygen_used_yesterday: req.body.b_total_oxygen_used_yesterday,
                b_total_oxygen_received_yesterday: req.body.b_total_oxygen_received_yesterday,
                b_total_oxygen_available: req.body.b_total_oxygen_available,
                d_total_oxygen_used_yesterday: req.body.d_total_oxygen_used_yesterday,
                d_total_oxygen_received_yesterday: req.body.d_total_oxygen_received_yesterday,
                d_total_oxygen_available: req.body.d_total_oxygen_available
...
app.post('/edit_patient',  auth,  HasRole("CENTER_DO"), ( req, res) => {
    console.log(req.body);
...
app.post('/add_referral',  auth,  HasRole("CENTER_DO"), ( req, res) => {
    ...
    Counter.findOneAndUpdate({
        ...
        if (err) {
            ...
            Center.findOne({
                center_id: req.body.from_center_id
            },(err, from_center_details) => {
                Center.findOne({
                    center_id: req.body.to_center_id
                },(err, to_center_details) => {
                ...
                from_center_details: from_center_details.facility_name,
                to_center_details: to_center_details.facility_name,
                ...
                bed_category: req.body.bed_category,
                "referrer_details.name":req.user.name,
                "referrer_details.phonenumber":req.user.phonenumber
            });
            referral.save();
            res.redirect("/patientmanagement");
            });});}
...
app.post('/accept_referral_rrt',  auth,  HasRole("CENTER_DO"), ( req, res) => {
    ...
    Referral.findOneAndUpdate({
        ...
        $set: {
            ...
            referred_approval_bed: req.body.bed_number,
            to_center_id: req.user.workplace,
            to_center_details: req.user.workplace_details,
        }
    }, (err, patient_updated) => {
        console.log(patient_updated)
        res.redirect("/in-referralsmanagement");
    });
...
app.post('/discharge_referral',  auth,  HasRole("CENTER_DO"), ( req, res) => {
    ...
    Referral.findOneAndUpdate({
        ...
        $set: {
            ...
            ambulance_driver_name: req.body.ambulance_driver_name,
            ambulance_driver_number: req.body.ambulance_driver_number,
        ...
        Patient.findOne({
            ...
            if (err) {
                ...
                res.redirect("/out-referralsmanagement");
            }
        });
    });
...
//** */
app.get("/patientdetails/:patient_id", auth,  function (req, res) {
            ...
            Center.findOne({
                ...
                if (1) {
                    ...
                    Referral.find({
                        ...
                        if (1) {
                            res.render(req.user.department+"/patient_details", {
                                element:req.user,
...
console.log("c1 execution started.")
    ...
    Center.updateMany({
        ...
        if(err){
            console.log("c1 execution error.")

        }

        if(!err){
            console.log("c1 execution done.")

            setTimeout(() => {
                return next();
                console.log("c1 executing next .")

            }, 2000);
        }
});}
function c2(req, res, next) {
    console.log("c2 executing started.")
    ...
    Sections.updateMany({
        ...
        if(err){
            console.log("c2 execution error.")

        }
        if(!err){
            setTimeout(() => {
                return next();
                console.log("c2 executing next.")

            }, 2000);
        }
});}
       
function c3(req, res, next) {
    console.log("c3 executing started.")

    Patient.find({
        discharge_status:"ACTIVE"
    }, (err, patient_list) => {
          for (let index = 0; index < patient_list.length; index++) {
              const element = patient_list[index];
            ...
            Center.findOneAndUpdate({
                center_id: element.active_bed_details.center_id
            }, {
                $push: {
                    active_patient_ids: {
                        patient_id: element.patient_id,
                        bed_category: element.active_bed_details.bed_category
                    }
                }
            }, (err, center_found) => {
                console.log(".")
                ...
                Sections.findOneAndUpdate({
                    section_id: element.active_bed_details.section_id
                }, {
                    $push: {
                        active_patient_ids: {
                            patient_id: element.patient_id,
                ...
                if(index==patient_list.length-1){
                    console.log("c3 executing done.")

                    setTimeout(() => {
                        console.log("c3 executing next.")
...
function c4(req, res, next) {

    console.log("c4 executing start.")


    Center.find({}, (err, centers_list) => {
        centers_list.forEach(element => {
            var i_b=0,g_b=0,o_b=0,h_b=0,ic_b=0,v_b=0;
            console.log(".")

            Sections.find({
                center_id:element.center_id
            }, async (err, section_list) => {
                for (let index = 0; index < section_list.length; index++) {
                    const section_details = section_list[index];
                    i_b+=section_details.no_isolation_beds;
                    g_b+=section_details.no_of_general_beds;
                    o_b+=section_details.no_of_oxygen_beds;
                    h_b+=section_details.no_hdu_beds;
                    ic_b+=section_details.no_of_icu_beds;
                    v_b+=section_details.no_of_ventilator_beds;
                    console.log(".")

                    if(index==section_list.length-1){
                        console.log("c4 execution ends")
                    ...
                    Center.findOneAndUpdate({
                        ...
                        $set: {
                            ...
                            no_ventilator: v_b,
                            no_hdu_bed: h_b,
                            no_isolation_bed: i_b
                        }, 
                        new: true,
                        setDefaultsOnInsert: true
                    }, (err, centers_list) => {
                        setTimeout(() => {
                            console.log("c4 execution next")
...
function c5(req, res, next) {
    console.log("c5 execution start")

setTimeout(() => {
    return next();
    console.log("c5 execution next")
